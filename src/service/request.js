...
export default async (url = "", params = {}, options = {}) => {
     ...
     */
    if(getUrlParam('appid')) config.appId = getUrlParam('appid');
    if(getUrlParam('privatekey')) config.privateKey = getUrlParam('privatekey');
    if (defaultOptions.needSign) {
        let { signUrl } = window.config;
        //如果 signUrl非空，优先使用signUrl  兼容 安卓 ios
        if (!isEmpty(signUrl) && !getUrlParam('appid') && !getUrlParam('privatekey')) {
